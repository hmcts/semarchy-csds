...
def get_release_package():
    ...
    url = f'{os.getenv("SemarchyBaseURL")}/named-query/CSDS/GetReleasePackagePNLD/GD'
    ...
    if len(records) == 1:
        ...
        rp_status = records[0].get("Status")

        return rp_id, rp_status

    # -----------------------------
    # Multiple Release Packages
    # -----------------------------
    rp_ids = [r.get("ReleasePackageID") for r in records]
    msg = f"Multiple Unpublished PNLD Release Packages found: {rp_ids}"
