...
def build_payload_get_offence_menu_summary(menu_details: dict) -> dict:
    """
    Extracts the required Offence Menu attributes from the Semarchy Named Query response.
    """

    payload = {
        "GetOffenceMenuSummaryRequest": {
            "Status": "ALL",                                        # TODO: Always set to ALL?
            "Title": menu_details.get("Name"),                      # TODO: Offence Menu name / title?  What if there are multiple OMs with the same name?
        },
        "AuditingInformation": {
...
def build_soap_xml_get_offence_menu_summary(json_obj: dict) -> str:
    """
    ...
    ET.SubElement(header, "wsa:Action").text = "getOffenceMenuSummary"
    ...
    payload = ET.SubElement(body, "ns62:GetOffenceMenuSummaryRequest")
