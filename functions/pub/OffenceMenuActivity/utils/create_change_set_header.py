...
def build_payload_create_change_set_header(menu_details: dict) -> dict:
    """
    Extracts the required Offence Menu attributes from the Semarchy Named Query response.
    """

    # Generate random NewRecordID by combining the OffenceMenuID with a random number to ensure uniqueness
    offence_menu_id = str(menu_details.get("OffenceMenuID"))
    remaining_length = 13 - len(str(offence_menu_id))
    rand_new_record_id = int(offence_menu_id + str(random.randint(10**(remaining_length-1), 10**remaining_length - 1)))
    ...
    payload = {
        "ChangeSetHeaderType": {
            ...
            "ChangeSetHeaderDescription": menu_details.get("Name", ""),                         # Human‑readable description for the Change Set (often includes reference code and date). Used for audit and UI display.
            ...
            "ReleasePackageDescription": menu_details.get("ReleasePackageDescription", ""),     # Descriptive text for the associated Release Package, if known.
            "ReferenceType": offence_menu_id,                                                   # Business reference identifying the item being changed (e.g. offence code). Used to prevent concurrent Change Sets for the same record.
            ...
            "ReferenceTypeDescription": menu_details.get("Name", ""),                           # Business description of the reference record, used in release notes and audit output.
            "Status": "Draft",                                                                  # Lifecycle status of the Change Set (Draft or Final). Creation always starts as Draft.
            "ChangeReason": menu_details.get("Notes", ""),                                      # Free‑text explanation of why the change is being made (audit and governance).
            "ParentRefType": "Offence Menus",                                                   # High‑level reference category (e.g. Offences). Determines which stored procedures and validation rules apply.
        },
        "AuditingInformation": {
