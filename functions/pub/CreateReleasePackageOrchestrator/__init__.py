import azure.functions as func
import azure.durable_functions as df
from datetime import datetime as dt, timezone, timedelta
import logging
import os

from utils.fail_handling import fail
def orchestrator(context: df.DurableOrchestrationContext):

    # Helper to avoid duplicate logs on replay
    def log_info(message: str):
        if not context.is_replaying:
            logging.info(message)

    # ----------------------------------------------------------
    # 1. Read Input
    # ----------------------------------------------------------
    rp_input = context.get_input() or {}

    rp_id = rp_input.get("ReleasePackageID")
    rp_urgent = rp_input.get("Urgent", "No")

    log_prefix = f"RELEASE PACKAGE ID: {rp_id} |"

    try:
        # ----------------------------------------------------------
        # 2. Conditional Delay (Timer)
        # ----------------------------------------------------------
        is_urgent = str(rp_urgent).strip().lower() in ["yes", "true", "1"]

        if is_urgent:
            delay_seconds = int(os.getenv("UrgentWaitPeriodSeconds"))
            deadline = context.current_utc_datetime + timedelta(seconds=delay_seconds)

            log_info(
                f'{log_prefix} URGENT | '
                f'Pausing execution for {delay_seconds} seconds. '
                f'Timer deadline = {deadline}'
            )

            yield context.create_timer(deadline)

        # ----------------------------------------------------------
        # 3. Fetch Release Package Details
        # ----------------------------------------------------------
        rp_details_list = yield context.call_activity(
            "GetCSDSDetailsActivity",
            {
                "ReleasePackageID": rp_id,
                "Type": "ReleasePackage"
            }
        )

        if not rp_details_list or not isinstance(rp_details_list, list):
            fail(f'{log_prefix} GET CSDS DETAILS | Invalid or no details received')

        rp_volume = len(rp_details_list)

        if rp_volume == 0:
            fail(f'{log_prefix} GET CSDS DETAILS | No details returned')

        if rp_volume > 1:
            fail(f'{log_prefix} GET CSDS DETAILS | Multiple details returned')

        log_info(f'{log_prefix} GET CSDS DETAILS | 1 detail record received')

        rp_details = rp_details_list[0]


        # ----------------------------------------------------------
        # 4. Validate Business Rules
        # ----------------------------------------------------------
        status = rp_details.get("Status")

        if status != "Publish to Live":
            msg = "Release Package is not set to Publish to Live"
            log_info(f'{log_prefix} GET CSDS DETAILS | {msg}')
            return {
                "Status": "STOPPED",
                "Reason": msg
            }
        

        publish_date_str = rp_details.get("PublishDate")
        if not publish_date_str:
            fail(f'{log_prefix} PUBLISH DATE VALIDATION | Missing Publish Date')

        try:
            publish_dt = dt.fromisoformat(publish_date_str.replace("Z", "+00:00"))
        except Exception:
            fail(
                f'{log_prefix} PUBLISH DATE VALIDATION | '
                f'Invalid Publish Date format "{publish_date_str}"'
            )

        current_dt = context.current_utc_datetime

        if publish_dt > current_dt:
            msg = f"Publish Date is in the future: {publish_dt}"
            log_info(f'{log_prefix} GET CSDS DETAILS | {msg}')
            return {
                "Status": "STOPPED",
                "Reason": msg
            }

        # ----------------------------------------------------------
        # 5. Perform RP Processing
        # ----------------------------------------------------------
        rp_results = yield context.call_activity(
            "CreateReleasePackageActivity",
            rp_details
        )
    

    
    except Exception as e:
       
        message = str(e).strip().split("\n")[0].strip()

        rp_results = {
            "ReleasePackageID": rp_id,
            "Status": "Error (Live)",
            "PublishingErrorMessage": message
        }



    # ----------------------------------------------------------
    # 6. Persist the Result
    # ----------------------------------------------------------
    persist_payload = {
        "ReleasePackage": [rp_results]
    }

    log_info(f'{log_prefix} POST CSDS DETAILS | Started')

    log_info(persist_payload)

    # Post to CSDS
    load_status = yield context.call_activity(
        "PostCSDSDetailsActivity",
        persist_payload
    )

    log_info(f'{log_prefix} POST CSDS DETAILS | Completed - {load_status}')

    # ----------------------------------------------------------
    # Final Structured Result
    # ----------------------------------------------------------
    if rp_results.get("Status")=="Error (Live)":
        fail(f"{log_prefix} CSDS INGESTION | Release Package Failed")

    return {
        "Status": "SUCCESS"
    }


# Azure Functions entrypoint
main = df.Orchestrator.create(orchestrator)