-- PROCEDURE: csds.proc_clone_offence_revision_children(numeric, character varying, numeric, character varying)

-- DROP PROCEDURE IF EXISTS csds.proc_clone_offence_revision_children(numeric, character varying, numeric, character varying);

CREATE OR REPLACE PROCEDURE csds.proc_clone_offence_revision_children(
	v_currentloadid numeric,
	v_username character varying,
	v_parentid numeric,
	v_authoringtype character varying)
LANGUAGE 'plpgsql'
AS $BODY$
...
ALTER PROCEDURE csds.proc_clone_offence_revision_children(numeric, character varying, numeric, character varying)
    OWNER TO postgres;

GRANT EXECUTE ON PROCEDURE csds.proc_clone_offence_revision_children(numeric, character varying, numeric, character varying) TO PUBLIC;

GRANT EXECUTE ON PROCEDURE csds.proc_clone_offence_revision_children(numeric, character varying, numeric, character varying) TO csds;

GRANT EXECUTE ON PROCEDURE csds.proc_clone_offence_revision_children(numeric, character varying, numeric, character varying) TO postgres;
